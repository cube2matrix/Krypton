...
# SUBMIT PYSPARK JOB
$SPARK_HOME/bin/spark-submit --conf spark.akka.frameSize=512 --conf spark.storage.memoryFraction=0.3 --conf spark.shuffle.memoryFraction=0.1 --conf spark.default.parallelism=$PNUM --conf spark.driver.maxResultSize=5g --num-executors 8 --driver-memory 5g --executor-memory 4g --executor-cores $executor_cores --jars $(echo /user/xli66/Krypton/Krypton/lib/*.jar | tr ' ' ',') --master $MASTER $PROG $ARGS $PNUM
